...
namespace UltrahapticsCoreAsset.Examples
{
    public class ParticleMotion : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                case 0:
                    GetComponent<Rigidbody2D>().linearVelocity = (Vector2.left + Vector2.up) * Speed;
                    break;
                case 1:
                    GetComponent<Rigidbody2D>().linearVelocity = (Vector2.right + Vector2.up) * Speed;
                    break;
                case 2:
                    GetComponent<Rigidbody2D>().linearVelocity = (Vector2.left + Vector2.down) * Speed;
                    break;
                default:
                    GetComponent<Rigidbody2D>().linearVelocity = (Vector2.right + Vector2.down) * Speed;
                    break;
            }
        }
